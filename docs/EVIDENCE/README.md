...
- **`tls-valid.png`** — real, non-self-signed certificate:
  ```bash
  curl -vI https://taskapp.rogers.<server-public-ip>.nip.io 2>&1 | grep -A5 "Server certificate"
  ```
...
- **`zero-downtime.log`** — unbroken 200s during a rolling update:
  ```bash
  while true; do curl -o /dev/null -s -w "%{http_code}\n" https://taskapp.rogers.<server-public-ip>.nip.io; sleep 1; done
  ```
...
- **`failover.png`** — app stays up after a node drain:
  ...
  curl -I https://taskapp.rogers.<server-public-ip>.nip.io
